...
func (t *Tree[T]) Insert(s string, v T) (ans T, result bool) {
	var parent *node[T]
	n := t.root
	search := s
	for {
...
func (t *Tree[T]) Delete(s string) (ans T, result bool) {
	var parent *node[T]
	var label byte
	n := t.root
	search := s
...
func (t *Tree[T]) Get(s string) (ans T, result bool) {
	n := t.root
	search := s
	for {
		// Check for key exhaution
...
func (t *Tree[T]) LongestPrefix(s string) (index string, ans interface{}, result bool) {
	var last *leafNode[T]
	n := t.root
	search := s
	for {
...
func (t *Tree[T]) Minimum() (index string, ans T, result bool) {
	n := t.root
	for {
		if n.isLeaf() {
			index = n.leaf.key
...
func (t *Tree[T]) Maximum() (index string, ans T, result bool) {
	n := t.root
	for {
